...
export function CardPergunta() {
  ...
  const [pergunta, setPergunta] = useState(localStorage.getItem("buscaPergunta") || "");
  ...
  useEffect(() => {
    const buscaPergunta = localStorage.getItem("buscaPergunta");
    if (buscaPergunta) {
      setPergunta(buscaPergunta);
    }
  }, []); 
  ...
  return (
    <div className="mx-5">
      <Card className="mt-6 h-max ">
        <CardBody>
          <Typography variant="h5" className="text-black text-3xl font-semibold mb-10">
            Tire sua dúvida
          </Typography>
          <TextField
